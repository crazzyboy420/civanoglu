        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{route('admin-location.store')}}" method="post" class="w-full flex justify-center">@csrf
                        <div class="w-6/12">
                            <label class="civanoglu-label" for="name">Location:</label>
                            <input class="civanoglu-input" type="text" id="name" name="location_name" value="{{old('name')}}" required>
                            @error('location_name')
